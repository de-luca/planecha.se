...
export class EternitiesMap extends Map {
    ...
    public planeswalk(coordinates: Coordinates): Array<Tile> {
        ...
        const activeTile = this.tiles.find((t) => {
            return t.coordinates.x === EternitiesMap.center.x 
                && t.coordinates.y === EternitiesMap.center.y;
        }) as Tile;
        ...
        this.tiles.forEach(t => {
            t.coordinates.x = t.coordinates.x + (xOffset * -1);
            t.coordinates.y = t.coordinates.y + (yOffset * -1);
        });

        // Remove all the plan that are too far
        this.tiles = this.tiles.filter((t) => {
            return Math.abs(t.coordinates.x) + Math.abs(t.coordinates.y) <= EternitiesMap.maxRange;
        });
