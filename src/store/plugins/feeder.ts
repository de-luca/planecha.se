...
function nameExtractor(store: Store): (payload?: Idable) => string {
  return function(payload?: Idable): string {
    if (payload?.mateId) {
      return store.getters.mates.get(payload?.mateId) as string;
    }
  ...
  return async function(mutation: MutationPayload, state: State): Promise<void> {
    switch (mutation.type) {
      ...
      case MutationTypes.PLANESWALK: {
        ...
        const message = `<b>${name(payload)}</b> ` +
      ...
      case MutationTypes.CUSTOM_PLANESWALK: {
        const payload = mutation.payload as PlaneswalkPayload;
        const message = `<b>${name(payload)}</b> planeswalked to ` +
      ...
      case MutationTypes.CHAOS: {
        ...
        store.state.feed.push(`<b>${name(payload)}</b> triggered <b>Chaos</b>`);
        break;
      }
